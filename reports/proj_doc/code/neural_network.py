...
def CNN_model():
    ...
    cnn_model.add(Conv2D(numFilters, kernel_size=3, strides=(1, 1), activation='relu'))
...
def train_model(model, x_train, y_train, x_valid, y_valid):
    ...
    # Training logging
    now = dt.now()
    log_file = "activityRec_train_log" + "_" + now.strftime("%d-%m-%Y_%H%M%S") + ".log"
    logfile_path = os.path.join(curlog_dir, log_file)
...
def save_best_models(max_acc_mod, min_loss_mod):
    ...
    loss_mod_path = os.path.join(best_mod_dir, "best_loss_model_" + dt.now().strftime("%d-%m-%Y_%H%M%S") + '.h5')
    acc_mod_path = os.path.join(best_mod_dir, "best_acc_model_" + dt.now().strftime("%d-%m-%Y_%H%M%S") + '.h5')
