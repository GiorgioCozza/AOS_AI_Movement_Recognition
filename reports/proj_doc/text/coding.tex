...
Before proceeding to read data from MEMS, it is required to configure the control registers. Activation and initialization are performed in a single step implemented by the \mintinline[bgcolor=whitesmoke]{C++}{init()} method in both sensors.\newline
...
\begin{center}
	...
	\end{figure}
\end{center}
It runs similarly for both LSM6DSL and LSM303AGR, setting the values to those registers that are more likely to be used for gathering purposes. In both classes, with slight differences, \mintinline[bgcolor=whitesmoke]{C++}{io_read()} and \mintinline[bgcolor=whitesmoke]{C++}{io_write()} act as a I2C interface to allow read and write operation respectively involving device, register addresses and bit masks\newline 
Crucial aspects of sensor configuration are:
\begin{itemize}
	\item \textit{Output data rate}: representing the frequency at which sensor provides new values
	\item \textit{Full scale value}: the maximum range of values that is possible to represent
	\item \textit{Sensitivity}: the minimum variation of the quantity to be measured that can be percepted by the MEMS. In some case changes according to the power mode in which the sensor is used.
\end{itemize}
All these values are set and shown in the initial steps of the program execution. 
