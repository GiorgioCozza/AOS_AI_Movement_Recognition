...
Before proceeding to read data from MEMS, it is required to configure the control registers. Activation and initialization are performed in a single step implemented by the \mintinline[bgcolor=whitesmoke]{C++}{init()} method. It runs similarly for both LSM6DSL and LSM303AGR, setting the values to those registers that are more likely to be used for gathering purpose. In both classes, with slight differences, \mintinline[bgcolor=whitesmoke]{C++}{io_read()} and \mintinline[bgcolor=whitesmoke]{C++}{io_write()} act as a I2C interface to allow read and write operation respectively involving sensor-specific features: 
